...
This assumes you have already built the Sneeze engine at least once, so its third-party dependencies are in place. Building the engine and its dependencies from scratch is a separate, much longer task; if you have not done that yet, follow the [Building Sneeze](../../docs/guides/building.md) guide first and then come back here. The commands below only regenerate the project files and compile the small tool - they never rebuild the dependencies.
...
On Linux or macOS, use `SignMsf` in place of `SignMsf.exe`. Everything else about signing, verifying, and the credentials is exactly as [Example 03](README.md) describes.
